...
class MicroProfileGetUserAlbumsTest {
    ...
    void whenServiceReturnsServerError_thenGatewayExceptionIsThrownWithResponse(final Hoverfly hoverfly) {
        ...
        configureHoverfly(hoverfly, baseUri, userId, serverError());
        final var client = new MicroProfileGetUserAlbums(buildClient(baseUri));
        // When
        final Executable executable = () -> client.perform(userId);
        // Then
    ...
    void whenServiceReturnsInvalidJson_thenGatewayExceptionIsThrown(final Hoverfly hoverfly) {
        ...
        final var userId = new Random().nextLong();
        final var baseUri = buildBaseUri();
        configureSuccessHoverfly(hoverfly, baseUri, userId, "{This is an invalid Json]");
        final var client = new MicroProfileGetUserAlbums(buildClient(baseUri));
        // When
        final Executable executable = () -> client.perform(userId);
        // Then
    ...
    void whenServiceReturnsData_thenClientReturnsExpected(final Hoverfly hoverfly) {
        ...
        final var baseUri = buildBaseUri();
        final var userId = new Random().nextLong();
        configureSuccessHoverfly(hoverfly, baseUri, userId, buildAlbumsResponse(userId));
        final var getUserAlbums = new MicroProfileGetUserAlbums(buildClient(baseUri));
        // When
        final var albums = getUserAlbums.perform(userId);
        // Then
    ...
    private static MicroProfileGetUserAlbums.MicroProfileGetUserAlbumsRestClient buildClient(final String baseUri) {
        return RestClientBuilder.newBuilder()
                .baseUri(URI.create(baseUri))
                .build(MicroProfileGetUserAlbums.MicroProfileGetUserAlbumsRestClient.class);
    }

    private static void configureSuccessHoverfly(
            final Hoverfly hoverfly, final String baseUri, final Long userId, final String body) {
        configureHoverfly(hoverfly, baseUri, userId, success(body, MediaType.APPLICATION_JSON));
    }

    private static void configureHoverfly(
            final Hoverfly hoverfly, final String baseUri, final Long userId, final ResponseBuilder responseBuilder) {
        hoverfly.simulate(dsl(
                service(baseUri)
                        .get("/albums").queryParam("userId", userId)
    ...
    private static String buildAlbumsResponse(final Long userId) {
        return String.format(ALBUMS_RESPONSE, userId, userId, userId);
    }
